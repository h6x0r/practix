...
describe("SubmissionsService", () => {
  ...
  const mockExecutionResult = {
    status: "passed",
    stdout:
      '{"tests":[{"name":"TestHelloWorld","passed":true}],"passed":1,"total":1}',
    stderr: "",
    compileOutput: "",
    time: "0.01",
    memory: 0,
  ...
  const mockCacheService = {
    ...
    getStats: jest.fn(),
    getRunValidation: jest
      .fn()
      .mockResolvedValue({ passed: true, testsPassed: 5 }),
  ...
  const mockAccessControlService = {
    ...
    getTaskAccess: jest
      .fn()
      .mockResolvedValue({ canSubmit: true, reason: null }),
  };
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: ResultFormatterService,
          useValue: mockResultFormatterService,
        },
  ...
  describe("create()", () => {
    describe("happy paths", () => {
      ...
      it("should execute code via Judge0 service with tests", async () => {
        ...
        expect(
          mockCodeExecutionService.executeSyncWithTests,
        ).toHaveBeenCalledWith("code", mockTask.testCode, "go", undefined);
      });
      ...
      it("should award XP on first successful submission", async () => {
        ...
        expect(mockGamificationService.awardTaskXp).toHaveBeenCalledWith(
          "user-123",
          "easy",
        );
    ...
    describe("edge cases", () => {
      ...
      it("should handle code execution timeout", async () => {
        ...
        mockResultFormatterService.formatMessage.mockReturnValue(
          "Time limit exceeded",
        );
      ...
      it("should use default priority if task has no course", async () => {
        ...
        expect(
          mockAccessControlService.getQueuePriority,
        ).not.toHaveBeenCalled();
      });
    });
    ...
    describe("test result parsing", () => {
      it("should delegate test parsing to TestParserService", async () => {
        ...
        mockCodeExecutionService.executeSyncWithTests.mockResolvedValue({
          status: "passed",
          stdout:
            '{"tests":[{"name":"Test1","passed":true},{"name":"Test2","passed":true}],"passed":2,"total":2}',
          stderr: "",
          compileOutput: "",
          time: "0.01",
          memory: 0,
      ...
      it("should handle failed tests and not award XP", async () => {
        ...
        mockCodeExecutionService.executeSyncWithTests.mockResolvedValue({
          status: "passed",
          stdout:
            '{"tests":[{"name":"Test1","passed":true},{"name":"Test2","passed":false}],"passed":1,"total":2}',
          stderr: "",
          compileOutput: "",
          time: "0.01",
          memory: 0,
  ...
  describe("runQuickTests()", () => {
    ...
    it("should handle compilation errors gracefully", async () => {
      ...
      const result = await service.runQuickTests(
        "task-123",
        "invalid code",
        "go",
      );
  ...
  describe("runQuickTests() - validation and persistence", () => {
    it("should mark task as validated when user passes 5 tests", async () => {
      ...
      expect(mockCacheService.setRunValidated).toHaveBeenCalledWith(
        "user-123",
        "task-123",
        5,
      );
    ...
    it("should continue even if run result save fails", async () => {
      ...
      mockPrismaService.runResult.upsert.mockRejectedValue(
        new Error("DB error"),
      );
  ...
  describe("awardXpIfFirstCompletion() - error handling", () => {
    it("should rethrow non-P2002 errors", async () => {
      ...
      mockAccessControlService.getQueuePriority.mockResolvedValue(1);
      mockCacheService.getRunValidation.mockResolvedValue({
        passed: true,
        testsPassed: 5,
      });
      mockCodeExecutionService.executeSyncWithTests.mockResolvedValue(
