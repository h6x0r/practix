...
public class Main {
  ...
  describe("scan() - system command execution", () => {
    it("should detect Python os.system", () => {
      ...
      expect(
        result.threats.some((t) => t.description.includes("System command")),
      ).toBe(true);
    });
  ...
  describe("scan() - file system access", () => {
    ...
    it("should detect sensitive directory access", () => {
      ...
      expect(result.isSafe).toBe(false);
      expect(
        result.threats.some((t) => t.description.includes("sensitive system")),
      ).toBe(true);
    });
  });
  ...
  describe("scan() - environment access", () => {
    it("should detect Python os.environ", () => {
      ...
      expect(
        result.threats.some((t) => t.description.includes("Environment")),
      ).toBe(true);
  ...
  describe("scan() - dynamic code execution", () => {
    it("should detect eval with parenthesis", () => {
      ...
      expect(
        result.threats.some((t) => t.description.includes("Dynamic code")),
      ).toBe(true);
    });
  ...
  describe("scan() - resource exhaustion", () => {
    it("should detect infinite loop (while true)", () => {
      ...
      expect(
        result.threats.some((t) => t.description.includes("infinite loop")),
      ).toBe(true);
    });
    ...
    it("should detect large memory allocation", () => {
      ...
      expect(
        result.threats.some((t) => t.description.includes("memory allocation")),
      ).toBe(true);
    });
  });
  ...
  describe("scan() - unsafe operations", () => {
    it("should detect Go unsafe package", () => {
      ...
      expect(result.threats.some((t) => t.description.includes("unsafe"))).toBe(
        true,
      );
    });
  });
  ...
  describe("scan() - reflection", () => {
    it("should detect Go reflect", () => {
      ...
      expect(
        result.threats.some((t) => t.description.includes("Reflection")),
      ).toBe(true);
    });
...
line4
  ...
  describe("scan() - disabled", () => {
    it("should pass all code when disabled", async () => {
      ...
      const disabledService =
        module.get<CodeScannerService>(CodeScannerService);
  ...
  describe("getThreatLevelDescription()", () => {
    it("should describe NONE", () => {
      expect(service.getThreatLevelDescription(ThreatLevel.NONE)).toBe(
        "No threats detected",
      );
    });

    it("should describe LOW", () => {
      expect(service.getThreatLevelDescription(ThreatLevel.LOW)).toBe(
        "Minor concerns (usually safe)",
      );
    });

    it("should describe MEDIUM", () => {
      expect(service.getThreatLevelDescription(ThreatLevel.MEDIUM)).toBe(
        "Suspicious patterns detected",
      );
    });

    it("should describe HIGH", () => {
      expect(service.getThreatLevelDescription(ThreatLevel.HIGH)).toBe(
        "Dangerous patterns detected",
      );
    });

    it("should describe CRITICAL", () => {
      expect(service.getThreatLevelDescription(ThreatLevel.CRITICAL)).toBe(
        "Critical security threat",
      );
    });

    it("should handle unknown level", () => {
      expect(service.getThreatLevelDescription("unknown" as ThreatLevel)).toBe(
        "Unknown",
      );
    });
  });
  ...
  describe("language-specific patterns", () => {
    ...
    it("should detect Go os.Exit pattern", () => {
      ...
      expect(
        result.threats.some((t) =>
          t.description.includes("Go process termination"),
        ),
      ).toBe(true);
    });
    ...
    it("should detect Go file operations", () => {
      ...
      expect(
        result.threats.some((t) => t.description.includes("Go file system")),
      ).toBe(true);
    });
  });
});
