...
describe('AdminService', () => {
  ...
  describe('getCourseAnalytics()', () => {
    beforeEach(() => {
      mockPrismaService.course.findMany.mockResolvedValue([
        { id: 'course-1', slug: 'go-basics', title: 'Go Basics', category: 'go' },
        { id: 'course-2', slug: 'java-basics', title: 'Java Basics', category: 'java' },
      ]);
    });
    ...
    it('should return course statistics', async () => {
      mockPrismaService.userCourse.groupBy
        .mockResolvedValueOnce([
          { courseSlug: 'go-basics', _count: { _all: 50 }, _avg: { progress: 75 } },
          { courseSlug: 'java-basics', _count: { _all: 30 }, _avg: { progress: 60 } },
        ])
        .mockResolvedValueOnce([
    ...
    it('should calculate completion rate correctly', async () => {
      mockPrismaService.userCourse.groupBy
        .mockResolvedValueOnce([
          { courseSlug: 'go-basics', _count: { _all: 100 }, _avg: { progress: 50 } },
        ])
        .mockResolvedValueOnce([
    ...
    it('should sort by popularity', async () => {
      mockPrismaService.userCourse.groupBy
        .mockResolvedValueOnce([
          { courseSlug: 'go-basics', _count: { _all: 30 }, _avg: { progress: 50 } },
          { courseSlug: 'java-basics', _count: { _all: 50 }, _avg: { progress: 60 } },
        ])
        .mockResolvedValueOnce([]);
  ...
  describe('getTaskAnalytics()', () => {
    beforeEach(() => {
      mockPrismaService.task.findMany.mockResolvedValue([
        { id: 'task-1', slug: 'hello-world', title: 'Hello World', difficulty: 'easy', isPremium: false },
        { id: 'task-2', slug: 'fizz-buzz', title: 'Fizz Buzz', difficulty: 'medium', isPremium: true },
      ]);
    });
    ...
    it('should calculate pass rate correctly', async () => {
      ...
      const hardestTask = result.hardestTasks.find(t => t.taskId === 'task-1');
      if (hardestTask) {
  ...
  describe('getSubscriptionStats()', () => {
    ...
    it('should calculate monthly revenue', async () => {
      ...
      mockPrismaService.subscriptionPlan.findMany.mockResolvedValue([
        { id: 'plan-1', name: 'Premium', slug: 'premium', type: 'global', priceMonthly: 999 },
      ]);
      mockPrismaService.payment.count.mockResolvedValue(100);
      mockPrismaService.payment.aggregate.mockResolvedValue({ _sum: { amount: 50000 } });
    ...
    it('should return payment statistics', async () => {
      ...
      mockPrismaService.payment.aggregate.mockResolvedValue({ _sum: { amount: 100000 } });
  ...
  describe('getAiUsageStats()', () => {
    it('should return total AI usage', async () => {
      mockPrismaService.aiUsage.aggregate.mockResolvedValue({ _sum: { count: 500 } });
    ...
    it('should return unique users count', async () => {
      mockPrismaService.aiUsage.aggregate.mockResolvedValue({ _sum: { count: 100 } });
      mockPrismaService.aiUsage.findMany
        .mockResolvedValueOnce([])
        .mockResolvedValueOnce([
          { userId: 'user-1' },
    ...
    it('should return daily usage breakdown', async () => {
      mockPrismaService.aiUsage.aggregate.mockResolvedValue({ _sum: { count: 25 } });
      mockPrismaService.aiUsage.findMany
        .mockResolvedValueOnce([
          { date: '2025-01-01', count: 10 },
          { date: '2025-01-02', count: 15 },
    ...
    it('should handle zero usage', async () => {
      mockPrismaService.aiUsage.aggregate.mockResolvedValue({ _sum: { count: null } });
