...
const CoursesPage = () => {
  ...
  useEffect(() => {
    ...
    // Load local progress
    const saved = localStorage.getItem(STORAGE_KEYS.STARTED_COURSES);
    if (saved) {
  ...
  const handleStartCourse = (e: React.MouseEvent, courseId: string) => {
    ...
    if (!startedCourses.includes(courseId)) {
      ...
      localStorage.setItem(STORAGE_KEYS.STARTED_COURSES, JSON.stringify(updated));
    }
  ...
  if (isLoading) {
      return <div className="max-w-7xl mx-auto pt-20 text-center text-gray-500 animate-pulse">Loading Catalog...</div>;
  }
