...
import {
  userCoursesService,
  UserCourse,
} from "@/features/courses/api/userCoursesService";
...
const MyTasksPage = () => {
  ...
  if (loading) {
    return (
      <div className="text-center pt-20 text-gray-500 animate-pulse">
        {tUI("myTasks.loadingProgress")}
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {courses.map((course) => {
          ...
          return (
            <div
              key={course.id}
              className="group relative bg-white dark:bg-dark-surface rounded-3xl border border-gray-100 dark:border-dark-border overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300"
            >
              {/* Gradient accent bar */}
              <div
                className={`h-1.5 bg-gradient-to-r ${theme.from} ${theme.to}`}
              />
              ...
              <div className="p-6">
                <div className="flex items-start gap-5">
                  ...
                  <div
                    className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${theme.from} ${theme.to} flex items-center justify-center text-2xl shadow-lg text-white flex-shrink-0 group-hover:scale-110 transition-transform duration-300`}
                  >
                    {course.icon}
                  </div>
              ...
              <div
                className={`absolute -bottom-16 -right-16 w-32 h-32 bg-gradient-to-br ${theme.from} ${theme.to} opacity-5 rounded-full blur-2xl group-hover:opacity-10 transition-opacity pointer-events-none`}
              />
            </div>
          );
        })}
