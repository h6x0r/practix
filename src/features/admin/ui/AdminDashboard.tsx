...
const AdminDashboard = () => {
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-center">
        ...
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{tUI('admin.loginRequired')}</h2>
        <p className="text-gray-500 dark:text-gray-400 max-w-sm mb-6">{tUI('admin.loginRequiredDesc')}</p>
  ...
  useEffect(() => {
    if (user && user.role === 'ADMIN') {
      ...
      Promise.all([
        ...
        .then(([stats, coursesResponse, tasksResponse, submissionsResponse, subscriptionsResponse]) => {
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-display font-bold text-gray-900 dark:text-white">{tUI('admin.title')}</h1>
        <p className="text-gray-500 dark:text-gray-400 mt-2">{tUI('admin.subtitle')}</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-white dark:bg-dark-surface p-6 rounded-2xl border border-gray-100 dark:border-dark-border shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{tUI('admin.activeSubscriptions')}</h3>
            ...
            <div className="text-3xl font-display font-bold text-gray-900 dark:text-white">{subscriptionStats.activeSubscriptions}</div>
            <div className="text-sm text-gray-500 mt-1">+{subscriptionStats.newSubscriptionsThisMonth} {tUI('admin.thisMonth')}</div>
          </div>

          {/* Subscriptions by Type - Grouped with fixed height scroll */}
          <div className="lg:col-span-2 bg-white dark:bg-dark-surface p-6 rounded-2xl border border-gray-100 dark:border-dark-border shadow-sm flex flex-col" style={{ height: '280px' }}>
            <h3 className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4 flex-shrink-0">{tUI('admin.subscriptionsByPlan')}</h3>
            {subscriptionStats.byPlan.length > 0 ? (
              <div className="flex-1 overflow-y-auto pr-2 space-y-4 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600 scrollbar-track-transparent">
                {/* Global Plans */}
                {(() => {
                  const globalPlans = subscriptionStats.byPlan.filter(p => p.planType === 'global');
                  ...
                  return (
                    <div>
                      <div className="text-xs font-semibold text-brand-500 uppercase tracking-wider mb-2 flex items-center gap-2 sticky top-0 bg-white dark:bg-dark-surface py-1">
                        ...
                        {tUI('admin.globalAccess')} ({globalPlans.reduce((sum, p) => sum + p.count, 0)})
                      </div>
                      <div className="space-y-2">
                        {globalPlans.map((plan) => (
                          <div key={plan.planId} className="flex items-center justify-between p-3 bg-brand-50 dark:bg-brand-900/20 rounded-xl">
                            <div className="font-medium text-gray-900 dark:text-white">{plan.planName}</div>
                            <div className="font-bold text-brand-600 dark:text-brand-400">{plan.count}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  return (
                    <div>
                      <div className="text-xs font-semibold text-purple-500 uppercase tracking-wider mb-2 flex items-center gap-2 sticky top-0 bg-white dark:bg-dark-surface py-1">
                        ...
                        {tUI('admin.courseAccess')} ({coursePlans.reduce((sum, p) => sum + p.count, 0)})
                      </div>
                      <div className="space-y-2">
                        {coursePlans.map((plan) => (
                          <div key={plan.planId} className="flex items-center justify-between p-2.5 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                            <div className="font-medium text-gray-900 dark:text-white text-sm truncate max-w-[200px]" title={plan.planName}>
                              {plan.planName}
                            </div>
                            <div className="font-bold text-purple-600 dark:text-purple-400 text-sm">{plan.count}</div>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <div className="flex-1 flex items-center justify-center text-gray-500">{tUI('admin.noActiveSubscriptions')}</div>
            )}
          </div>
          ...
          <div className="bg-gradient-to-br from-brand-600 to-purple-600 p-6 rounded-2xl shadow-lg text-white">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-white/80 uppercase tracking-wider">{tUI('admin.monthlyRevenue')}</h3>
            ...
            <div className="text-3xl font-display font-bold">
              {(subscriptionStats.totalMonthlyRevenue / 100).toLocaleString()} <span className="text-lg">UZS</span>
            </div>
            <div className="text-sm text-white/70 mt-1">{tUI('admin.estimatedMonthly')}</div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white dark:bg-dark-surface p-6 rounded-3xl border border-gray-100 dark:border-dark-border shadow-sm">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-6">{tUI('admin.submissionsByStatus')}</h2>
          {submissionsByStatus.length > 0 ? (
            ...
            <div className="text-center text-gray-500 py-8">{tUI('admin.noSubmissions')}</div>
          )}
        </div>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-dark-surface p-6 rounded-3xl border border-gray-100 dark:border-dark-border shadow-sm">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-6">{tUI('admin.submissionsByDay')}</h2>
      ...
      <div className="bg-white dark:bg-dark-surface p-6 rounded-3xl border border-gray-100 dark:border-dark-border shadow-sm flex flex-col" style={{ height: '420px' }}>
        <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4 flex-shrink-0">{tUI('admin.coursePopularity')}</h2>
        {courseAnalytics.length > 0 ? (
          <div className="flex-1 overflow-hidden flex flex-col">
            ...
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-dark-border pb-3 mb-2 pr-4">
              <div className="flex items-center gap-4">
                ...
                <div className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{tUI('admin.course')}</div>
              </div>
              <div className="flex items-center gap-4 flex-shrink-0">
                <div className="w-20 text-center text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{tUI('admin.enrolled')}</div>
                <div className="w-24 text-center text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{tUI('admin.completed')}</div>
              </div>
            </div>
            {/* Scrollable Body */}
                      ...
                      >
                        <div className="flex items-center gap-4 min-w-0 flex-1">
                          ...
                          <div className="min-w-0 flex-1">
                            <h3 className="font-medium text-gray-900 dark:text-white truncate">{getLocalizedTitle(course)}</h3>
                            <span className="text-xs text-gray-500 dark:text-gray-400">{course.category}</span>
                          </div>
                        </div>
                        {/* Stats */}
                        <div className="flex items-center gap-4 flex-shrink-0">
                          <div className="w-20 text-center text-lg font-bold text-brand-500">{course.totalEnrolled}</div>
                          <div className={`w-24 text-center text-lg font-bold ${course.completionRate >= 50 ? 'text-green-500' : 'text-amber-500'}`}>
          ...
          <div className="flex-1 flex items-center justify-center text-gray-500">{tUI('admin.noEnrollmentData')}</div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-dark-surface p-6 rounded-3xl border border-gray-100 dark:border-dark-border shadow-sm flex flex-col" style={{ height: '380px' }}>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-6 flex-shrink-0">{tUI('admin.topCoursesByCompletion')}</h2>
          <div className="flex-1 overflow-hidden flex flex-col">
            {/* Header Row */}
            <div className="flex items-center border-b border-gray-100 dark:border-dark-border pb-3 mb-2">
              <div className="flex-1 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{tUI('admin.course')}</div>
              <div className="w-24 text-center text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{tUI('admin.completions')}</div>
              <div className="w-24 text-center text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{tUI('admin.rate')}</div>
            </div>
            {/* Scrollable Body */}
            <div className="flex-1 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600 scrollbar-track-transparent">
              <div className="divide-y divide-gray-100 dark:divide-dark-border">
                {[...courseAnalytics]
                  ...
                  .map((course) => (
                  <div key={course.courseId} className="flex items-center py-3 hover:bg-gray-50 dark:hover:bg-dark-bg/50 transition-colors -mx-2 px-2 rounded-lg">
                    <div className="flex-1 text-sm text-gray-900 dark:text-white font-medium truncate pr-4">{getLocalizedTitle(course)}</div>
                    <div className="w-24 text-center text-sm text-gray-600 dark:text-gray-300">{course.completed}</div>
        ...
        <div className="bg-white dark:bg-dark-surface p-6 rounded-3xl border border-gray-100 dark:border-dark-border shadow-sm flex flex-col" style={{ height: '380px' }}>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-6 flex-shrink-0">{tUI('admin.hardestTasks')}</h2>
          <div className="flex-1 overflow-hidden flex flex-col">
            {/* Header Row */}
            <div className="flex items-center border-b border-gray-100 dark:border-dark-border pb-3 mb-2">
              <div className="flex-1 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{tUI('admin.task')}</div>
              <div className="w-24 text-center text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{tUI('admin.submissions')}</div>
              <div className="w-28 text-center text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{tUI('admin.passRate')}</div>
            </div>
            {/* Scrollable Body */}
            <div className="flex-1 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600 scrollbar-track-transparent">
              <div className="divide-y divide-gray-100 dark:divide-dark-border">
                {[...hardestTasks]
                  ...
                  .map((task) => (
                  <div key={task.taskId} className="flex items-center py-3 hover:bg-gray-50 dark:hover:bg-dark-bg/50 transition-colors -mx-2 px-2 rounded-lg">
                    <div className="flex-1 text-sm text-gray-900 dark:text-white font-medium truncate pr-4">{task.taskTitle}</div>
                    <div className="w-24 text-center text-sm text-gray-600 dark:text-gray-300">{task.totalSubmissions}</div>
