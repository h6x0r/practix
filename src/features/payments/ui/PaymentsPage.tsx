  ...
  useEffect(() => {
    if (user) {
        paymentService.getPaymentHistory()
          ...
          .catch((error) => {
            log.error('Failed to load payment history', error);
            setLoading(false);
          });
    }
  }, [user]);
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          {user?.isPremium ? (
            ...
            <div>
                ...
                <p className="text-gray-400 max-w-md">You are currently on the free plan. Upgrade to unlock the full potential of Practix.</p>
            </div>
          )}
          ...
          {user?.isPremium ? (
             ...
             <button className="px-8 py-3 bg-gradient-to-r from-brand-600 to-purple-600 hover:from-brand-500 hover:to-purple-500 text-white font-bold rounded-xl shadow-lg shadow-brand-500/25 transition-all transform hover:-translate-y-0.5">
                Upgrade Plan
             </button>
          )}
        </div>
