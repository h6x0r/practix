...
const PricingPage: React.FC = () => {
  ...
  const handleSelectPlan = (planSlug?: string) => {
    if (!user) {
      navigate('/login?redirect=/payments' + (planSlug ? `?plan=${planSlug}` : ''));
      return;
    }
    ...
    <div className="max-w-6xl mx-auto py-8 px-4">
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-16">
        ...
        <div className="bg-white dark:bg-dark-surface rounded-3xl border border-gray-200 dark:border-dark-border p-8 flex flex-col">
          ...
          <div className="mb-6">
            ...
            <div className="text-sm text-gray-500">{tUI('pricing.forever')}</div>
          </div>

          <ul className="space-y-3 mb-8 flex-1">
            {['3 бесплатных курса', '5 AI запросов/день', 'Playground', 'Leaderboard'].map(
              (feature) => (
        ...
        <div className="bg-white dark:bg-dark-surface rounded-3xl border border-gray-200 dark:border-dark-border p-8 flex flex-col">
          ...
          <div className="mb-6">
            ...
            <div className="text-sm text-gray-500">
              {billingCycle === 'yearly' ? tUI('pricing.perYear') : tUI('pricing.perMonth')}
            </div>
          </div>

          <ul className="space-y-3 mb-8 flex-1">
            {['1 курс на выбор', '30 AI запросов/день', 'Сертификат', 'Приоритетная поддержка'].map(
              (feature) => (
                <li key={feature} className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                  <IconCheck className="w-4 h-4 text-green-500 flex-shrink-0" />
                  {feature}
        ...
        <div className="bg-gradient-to-br from-brand-600 to-purple-600 rounded-3xl p-8 flex flex-col text-white relative overflow-hidden">
          ...
          <div className="mb-6">
            ...
            <p className="text-white/80 text-sm mt-2">{tUI('pricing.premiumDesc')}</p>
          </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-dark-border">
              {FEATURES.map((feature) => (
                <tr key={feature.key} className="hover:bg-gray-50 dark:hover:bg-dark-bg/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-center">
                    {typeof feature.course === 'boolean' ? (
                      ...
                      <span className="text-sm text-gray-600 dark:text-gray-400">{feature.course}</span>
                    )}
                  </td>
      ...
      <div className="mt-16">
        ...
        <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
          {[
            ...
            >
              <h3 className="font-bold text-gray-900 dark:text-white mb-2">{tUI(item.q)}</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">{tUI(item.a)}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mt-16 text-center">
        ...
        <p className="text-gray-500 dark:text-gray-400">
          ...
          <a href="mailto:support@practix.uz" className="text-brand-600 hover:underline">
            support@practix.uz
          </a>
        </p>
      </div>
