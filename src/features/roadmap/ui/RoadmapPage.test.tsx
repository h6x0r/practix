  ...
  useUITranslation: () => ({
    tUI: (key: string) => {
      const translations: Record<string, string> = {
        ...
        'roadmap.introDescription': 'Answer a few questions and get a personalized learning path',
        ...
        'roadmap.regeneratePremiumDesc': 'Upgrade to create unlimited personalized roadmaps',
        'roadmap.regenerateModalTitle': 'Regenerate Your Roadmap',
        'roadmap.regenerateModalDesc': 'Create a new personalized learning path based on updated preferences',
...
describe('RoadmapPage', () => {
    ...
    phases: [
      {
        ...
        steps: [
          { id: 'step-1', title: 'Variables', type: 'task', status: 'completed', deepLink: '/task/go-vars', durationEstimate: '15m' },
          { id: 'step-2', title: 'Functions', type: 'task', status: 'pending', deepLink: '/task/go-func', durationEstimate: '20m' },
        ],
      },
    ],
  };
  ...
  beforeEach(() => {
    ...
    vi.mocked(roadmapService.generateVariants).mockResolvedValue({ variants: mockVariants });
  ...
  describe('initial state', () => {
    ...
    it('should show intro screen when no roadmap exists', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('AI-Powered Learning Roadmap')).toBeInTheDocument();
      });
  ...
  describe('error handling', () => {
    it('should handle API error gracefully and show intro', async () => {
      vi.mocked(roadmapService.getUserRoadmap).mockRejectedValue(new Error('API Error'));
  ...
  describe('roadmap display', () => {
    ...
    it('should display Goal Achieved section', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/Ready for Backend Developer/)).toBeInTheDocument();
      });
    });
  ...
  describe('regenerate flow', () => {
    ...
    it('should show premium gate modal when canRegenerate is false and not premium', async () => {
      ...
      vi.mocked(roadmapService.getUserRoadmap).mockResolvedValue(nonRegenerateRoadmap);
      ...
      const regenerateButton = screen.getByText(/Regenerate \(Premium\)/).closest('button');
    ...
    it('should show premium required banner when canRegenerate is false', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Regeneration requires Premium/)).toBeInTheDocument();
      });
    });
  });
});
