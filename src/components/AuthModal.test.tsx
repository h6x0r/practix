...
describe('AuthModal', () => {
  ...
  describe('rendering', () => {
    ...
    it('should render login form by default', () => {
      ...
      expect(screen.queryByPlaceholderText('Alex Developer')).not.toBeInTheDocument();
    });
  ...
  describe('login submission', () => {
    it('should call authService.login on form submit', async () => {
      ...
      vi.mocked(authService.login).mockResolvedValue({ user: mockUser, token: 'token123' });

      renderModal();

      await userEvent.type(screen.getByPlaceholderText('alex@example.com'), 'test@example.com');
  ...
  describe('register submission', () => {
    it('should call authService.register on register submit', async () => {
      ...
      vi.mocked(authService.register).mockResolvedValue({ user: mockUser, token: 'token123' });

      renderModal();
      fireEvent.click(screen.getByText('Sign Up'));

      await userEvent.type(screen.getByPlaceholderText('Alex Developer'), 'New User');
      await userEvent.type(screen.getByPlaceholderText('alex@example.com'), 'new@example.com');
      await userEvent.type(screen.getByPlaceholderText('••••••••'), 'password123');
  ...
  describe('error handling', () => {
    it('should show toast on API error', async () => {
      vi.mocked(authService.login).mockRejectedValue(new ApiError('Invalid credentials', 401));
