...
const Sidebar = () => {
  ...
  return (
    ...
    >
      <div
        className={`h-16 flex items-center ${isCollapsed ? "justify-center" : "justify-between px-6"}`}
      >
        {/* Logo Area */}
        {!isCollapsed ? (
          <Link
          ...
          <Link
            to="/"
            className="w-8 h-8 bg-gradient-to-br from-brand-500 to-brand-600 rounded-lg flex items-center justify-center text-white font-display font-black text-lg shadow-lg"
          >
            P
          </Link>
        )}
        ...
        {isLoading
          ? // Skeleton Loader for Nav
            Array.from({ length: 5 }).map((_, i) => (
              <div
                key={i}
                className="h-10 bg-gray-100 dark:bg-dark-bg/50 rounded-xl animate-pulse mb-2 mx-1"
              ></div>
            ))
          : navItems.map((item) => {
              ...
              return (
                ...
                >
                  <IconComponent
                    className={`w-5 h-5 flex-shrink-0 ${isActive(item.path) ? "text-brand-600 dark:text-brand-400" : ""}`}
                  />
      ...
      {/* User Mini Profile */}
      <div
        className={`p-4 border-t border-gray-200 dark:border-dark-border ${isCollapsed ? "flex justify-center" : ""}`}
      >
        {!isCollapsed ? (
...
const Header = () => {
  ...
  const getPageTitle = () => {
    ...
    if (path.startsWith("/payments") || path.startsWith("/premium"))
      return tUI("nav.payments");
  ...
  return (
    <header className="h-16 bg-white/80 dark:bg-dark-surface/80 backdrop-blur-md border-b border-gray-200 dark:border-dark-border flex items-center justify-between px-6 transition-colors duration-300 sticky top-0 z-10">
      <div className="flex items-center text-gray-400 text-sm font-medium">
        ...
        <span className="mx-2 text-gray-300 dark:text-gray-700 hidden sm:inline">
          /
        </span>
      ...
      <div className="flex items-center gap-4">
        ...
        {!user?.isPremium && (
          <Link
            to="/premium"
            className="hidden sm:flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-brand-600 to-purple-600 hover:from-brand-500 hover:to-purple-500 text-white text-xs font-bold rounded-lg shadow-lg shadow-brand-500/25 transition-all transform hover:-translate-y-0.5"
          >
        ...
        {user ? (
          <div
            ...
            className="flex items-center gap-3 pl-4 border-l border-gray-200 dark:border-dark-border"
          >
            ...
            {user.avatarUrl ? (
              <img
                src={user.avatarUrl}
                alt="Avatar"
                className="w-9 h-9 rounded-full ring-2 ring-gray-100 dark:ring-dark-border"
              />
            ) : (
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-brand-500 to-purple-600 text-white flex items-center justify-center font-bold shadow-sm">
                {user.name.charAt(0)}
              </div>
            )}
