...
test.describe("Dashboard", () => {
  test.describe("Dashboard - Unauthenticated", () => {
    test("should show dashboard or auth state for unauthenticated user", async ({
      page,
    }) => {
      ...
      // Check that we're on a valid page (baseURL or auth-related)
      const baseUrl = process.env.E2E_BASE_URL || "http://localhost:3000";
      const isValidUrl =
        url.startsWith(baseUrl) ||
        url.includes("auth") ||
        url.includes("login");
      expect(isAuthOverlay || isDashboard || isValidUrl).toBe(true);
    });

    test("should not show dashboard content for unauthenticated user", async ({
      page,
    }) => {
      await page.context().clearCookies();
      await page.goto("/");
      ...
      const totalSolved = await dashboardPage.totalSolvedCard
        .isVisible()
        .catch(() => false);
      const hoursSpent = await dashboardPage.hoursSpentCard
        .isVisible()
        .catch(() => false);
  ...
  test.describe("Dashboard - Free User", () => {
    ...
    test("should display total solved stat", async ({ page }) => {
      ...
      const isVisible = await solvedText
        .first()
        .isVisible()
        .catch(() => false);
    ...
    test("should display streak information", async ({ page }) => {
      ...
      const isVisible = await streakText
        .first()
        .isVisible()
        .catch(() => false);
    ...
    test("should display activity chart or section", async ({ page }) => {
      ...
      const hasActivityText = await activitySection
        .first()
        .isVisible()
        .catch(() => false);
      const hasChart = await chartElement
        .first()
        .isVisible()
        .catch(() => false);
    ...
    test("should display recent activity or empty state", async ({ page }) => {
      ...
      expect(activityCount > 0 || isEmptyShown || activityCount === 0).toBe(
        true,
      );
    });
  ...
  test.describe("Dashboard Error Handling", () => {
    ...
    test("should handle API errors gracefully", async ({ page }) => {
      ...
      // Page should not crash - check URL is valid (homepage or baseURL)
      const url = page.url();
      const baseUrl = process.env.E2E_BASE_URL || "http://localhost:3000";
      expect(url.endsWith("/") || url.startsWith(baseUrl)).toBe(true);
    });
    ...
    test("should handle network errors gracefully", async ({ page }) => {
      ...
      // Page should handle error gracefully - still on valid URL
      const baseUrl = process.env.E2E_BASE_URL || "http://localhost:3000";
